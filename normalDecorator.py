...
class ArcadeWindow(arcade.Window):
    ...
    def on_key_release(self, key, key_modifiers):
        game = self.registry.get('game')
        for func in self.registry['key_release']:
            if game is not None:
                # Pass game instance as self
                func(game, key, key_modifiers)
            else:
    ...
    def on_mouse_motion(self, x, y, dx, dy):
        game = self.registry.get('game')
        for func in self.registry['mouse_moution']:
            if game is not None:
                func(game, x, y, dx, dy)
            else:
                sig = signature(func)
                if 'window' in sig.parameters:
                    func(self.registry['window'], x, y, dx, dy)
                else:
                    func(x, y, dx, dy)

    def on_mouse_release(self, x, y, button, modifiers):
        game = self.registry.get('game')
        for func in self.registry['mouse_release']:
            if game is not None:
                func(game, x, y, button, modifiers)
            else:
                sig = signature(func)
                if 'window' in sig.parameters:
                    func(self.registry['window'], x, y, button, modifiers)
                else:
                    func(x, y, button, modifiers)

    def on_mouse_press(self, x, y, button, modifiers):
        game = self.registry.get('game')
        for func in self.registry['mouse_press']:
            if game is not None:
                func(game, x, y, button, modifiers)
            else:
                sig = signature(func)
...
class decorator(arcade.Window):
    ...
    @classmethod
    def key_release(cls, original_function):
        cls.registry['key_release'].append(original_function)
        return original_function

    @classmethod
    def mouse_moution(cls, original_function):
        cls.registry['mouse_moution'].append(original_function)
        return original_function
    @classmethod
    def mouse_release(cls, original_function):
        cls.registry['mouse_release'].append(original_function)
        return original_function
    @classmethod
    def mouse_press(cls, original_function):
        cls.registry['mouse_press'].append(original_function)
        return original_function
